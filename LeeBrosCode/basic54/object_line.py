...
1. 키가 더 큰 학생이 앞에 와야 합니다.
2. 키가 동일하다면, 몸무게가 더 큰 학생이 앞에 와야 합니다.
3. 키와 몸무게가 동일하다면, 번호가 작은 학생이 앞에 와야 합니다.
...
1 ≤ N ≤ 1,000
출력 형식
규칙에 따라 정렬한 이후의 결과를 N개의 줄에 걸쳐 출력합니다. 각 줄에는 정렬한 이후 N명의 학생의 정보를 순서대로 한 줄에 하나씩 출력합니다. 각 줄에는 각 학생의 키, 몸무게, 그리고 번호를 공백을 사이에 두고 출력합니다.
